...
public partial class NumericInputControl : UserControl
{
    ...
    public static readonly StyledProperty<double> ControlWidthProperty =
        AvaloniaProperty.Register<NumericInputControl, double>(nameof(ControlWidth), defaultValue: 200);

    public static readonly StyledProperty<double> ControlHeightProperty =
        AvaloniaProperty.Register<NumericInputControl, double>(nameof(ControlHeight), defaultValue: 100);
    ...
    private void OnResizeGripMoved(object? sender, PointerEventArgs e)
    {
        if (_isResizing && sender is Border grip)
        {
            ...
            if (_resizeMode == "bottomright")
            {
                ...
                var newHeight = Math.Max(80, Math.Min(400, _resizeStartHeight + deltaY));
